...
object TypeChecker {
  ...
  case class WrongTypeError(actualType: Type, expectedType: Type, index: Int) extends TypeError
  case class TypeNotInListError(actualType: Type, expectedTypes: Seq[Type], index: Int) extends TypeError
  case class WrongArgumentAmountError(actual: Int, expected: Int, index: Int) extends TypeError
  case class UndefinedNameError(name: String, index: Int) extends TypeError
  ...
  private def typeCheck(expr: Expr, c: Context): Either[TypeError, Type] =
    expr match {
  ...
  private def typeCheck(treeNode: SyntaxTreeNode, c: Context): Either[TypeError, Unit] =
    treeNode match {
      case program: Program =>
        for {
          _ <- typeCheck(program.mainClass, c)
          _ <- typeCheckAll(program.classDecls, c)
        } yield ()
  ...
  private def assertNotMultidimArray(arrayLookup: ArrayLookup): Either[TypeError, Unit] = arrayLookup.array match {
