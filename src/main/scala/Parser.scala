...
object Parser {
  ...
  def parse(s: String): Parsed[Program] = fastparse.parse(s, program(_), verboseFailures = true)

  /*
  We use cuts (~/) liberally in order to improve the quality of errors. Anywhere there's an Either or Repeat,
  the children have cuts once the parse has progressed far enough backtracking isn't an option.
   */
}